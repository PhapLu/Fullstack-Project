...
export function CartProvider({ children }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const bootLoggedIn =
        !!userId && !isLogin && !hydratedKeysRef.current.has(storageKey);

      // 1) Hydrate from local for *this* key
      if (!hydratedKeysRef.current.has(storageKey)) {
        const parsed = readLocal(storageKey) || readLocal(makeKey(null));
        if (!canceled) {
          parsed.items = (parsed.items || []).filter((x) => x && x.id);
