    ...
    <div className={styles["auth__card"]}>
      <div className={styles["auth__panel"]}>
        <div className={styles["auth__header"]}>
          ...
          {isSignup && <span className={styles["auth__name"]}>Bloomart</span>}
        </div>
        ...
        {!isSignup && <img className={styles["auth__logo-form"]} src={logo} alt="Logo" />}
        ...
        <form className={styles["auth__form"]} onSubmit={onSubmit}>
          ...
          <label className={styles.field}>
            ...
            <button type="button" className={styles.eye} onClick={() => setShow1((v) => !v)}>
            ...
            <>
              <label className={styles.field}>
                ...
                <input
                  ...
                  required
                />
                <button type="button" className={styles.eye} onClick={() => setShow2((v) => !v)}>
          ...
          <button className={`${styles.btn} ${styles["btn-primary"]} ${styles["auth__submit"]}`} type="submit">
            {isSignup ? "Create Account" : "Sign in"}
          </button>

          <div className={styles.help}>
            {isSignup ? "Have an account? " : "Don’t have an account yet? "}
            <Link className={styles.link} to={`/auth/${isSignup ? "signin" : "signup"}/${role}`}>
            ...
            <div className={styles["help__role-selection"]}>
              {["customer", "vendor", "shipper"]
                ...
                .map((r, i) => (
                  <span key={r}>
                    {i > 0 && " / "}
                    <Link className={styles["link-mode"]} to={`/auth/${mode}/${r}`}>
                      {cap(r)}
                    </Link>
                  </span>
                ))}
