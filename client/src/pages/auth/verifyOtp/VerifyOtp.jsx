...
import { signIn, openSocket, fetchMe } from "../../../store/slices/authSlices";
...
export default function VerifyOtp() {
  ...
  const validate = () => {
    ...
    if (!registerInputs?.username || !registerInputs?.password) {
      e.register =
        "Thiếu username hoặc mật khẩu đăng ký. Vui lòng đăng ký lại.";
    }
    return e;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (action.meta.requestStatus === "fulfilled") {
        ...
        setErrors((prev) => ({
          ...prev,
          server: action.payload || "Đăng nhập thất bại sau khi xác thực OTP.",
        }));
      }
    } catch (err) {
      setErrors((prev) => ({
    ...
    >
      ...
      <div className={styles.formActions}>
        <button
          ...
          disabled={submitting}
        >
          {submitting ? <span className="btn-spinner"></span> : "Verify"}
        </button>
      </div>
    </form>
  );
