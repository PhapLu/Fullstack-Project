...
export default function VendorProfile() {
  ...
  const validateVendorProfile = (f) => {
    ...
    if (!minLength(f.vendorProfile?.businessAddress, 5)) {
      e.businessAddress = "Business address is required (min 5 characters).";
    }
                ...
                return (
                  <button
                    ...
                    className={`${styles.tab} ${
                      activeTab === key ? styles["tab--active"] : ""
                    }`}
              ...
              <div className={styles.grid}>
                {products?.map((p) => (
                  <ProductCard
                    key={p._id}
                    product={p}
                    onDelete={isOwner ? () => askDelete(p) : undefined}
                  />
                ))}
              </div>
            )}
              ...
              <div className={styles["profile-card"]}>
                <div className={styles["profile-card-head"]}>
                  {!isEditingProfile && isOwner && (
                    <div className={styles.actions}>
                      <button
                        type="button"
                        className={styles["icon-btn-profile"]}
                        onClick={() => isOwner && setIsEditingProfile(true)}
                        disabled={!isOwner}
                      >
                        Edit information
                      </button>
                ...
                <div className={styles.profileTable}>
                  ...
                  <div className={styles.row}>
                    ...
                    <div className={styles.valueFull}>
                      {!isEditingProfile ? (
                        profile?.vendorProfile?.businessAddress || (
                          <span className={styles.muted}>—</span>
                        )
                      ) : (
                        <>
