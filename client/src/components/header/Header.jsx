        ...
        <header className={styles.header} id="site-header">
            ...
            <div className={styles.header__top}>
                ...
                <nav className={styles.header__auth}>
                    {!user ? (
                        <>
                            <Link to="/auth/signin" className={styles["header__link--signin"]}>Sign In</Link>
                            <span>│</span>
                        ...
                        <div className={styles.userBox}>
                            ...
                            <div className={styles.menuBar} role="menu" aria-label="User menu">
                                ...
                                {user?.role === "vendor" ? (
                                    <button type="button" className={styles.menuItem} onClick={() => navigate("/vendordashboard")}>
                                        <FontAwesomeIcon icon={["fas", "gauge"]} className={styles.menuIcon} />
                                        <span>Vendor Dashboard</span>
                                    </button>
                                ) : user?.role === "shipper" ? (
                                    <button type="button" className={styles.menuItem} onClick={() => navigate(`/distributionHub/${user.shipperProfile.assignedHub._id}`)}>
                                        <FontAwesomeIcon icon={["fas", "gauge"]} className={styles.menuIcon} />
                                        <span>Shipper Dashboard</span>
                                    </button>
                                ) : user?.role === "admin" ? (
                                    <button type="button" className={styles.menuItem} onClick={() => navigate("/admin")}>
                                        <FontAwesomeIcon icon={["fas", "gauge"]} className={styles.menuIcon} />
                                        <span>Admin Dashboard</span>
                                    </button>
                                ) : (
                                    <button type="button" className={styles.menuItem} onClick={() => navigate(`/user/${user?._id}/order-history`)}>
                                        <FontAwesomeIcon icon={["fas", "receipt"]} className={styles.menuIcon} />
                                        <span>Order History</span>
                                    </button>
                                )}

                                <button type="button" className={styles.menuItem} onClick={() => navigate(`/${user?.role === "vendor" ? "vendor" : "user"}/${user?._id}`)}>
            ...
            <div className={styles.header__searchRow}>
                ...
                <div className={styles.header__search} role="search">
                    ...
                    />
                    <button type="button" aria-label="search" className={styles.header__searchBtn} onClick={handleSearch}>
                        <FontAwesomeIcon icon={["fas", "magnifying-glass"]} style={{ color: "#ffffff" }} />
                    </button>
                </div>
