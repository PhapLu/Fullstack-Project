...
export const productPublicUrlFor = (filename) => `/public/products/${filename}`;
...
class ProductService {
  ...
  static createProduct = async (req) => {
    ...
    if (title.trim().length < 10 || title.trim().length > 20)
      throw new BadRequestError("Title must be between 10 and 20 characters");

    // Description: required, ≤500 chars
    if (!description?.trim())
      throw new BadRequestError("Description is required");
    if (description.trim().length > 500)
      throw new BadRequestError("Description must be at most 500 characters");
