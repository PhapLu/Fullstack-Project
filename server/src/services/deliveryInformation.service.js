...
class DeliveryInformationService {
  ...
  static createDeliveryInformation = async (req) => {
    ...
    const existingDefault = await DeliveryInformation.findOne({
      customerId: userId,
      isDefault: true,
    });
    if (!isDefault) {
  ...
  static readDeliveryInformation = async (req) => {
    ...
    if (!shipper || shipper.role !== "shipper")
      throw new AuthFailureError(
        "You are not authorized to view DeliveryInformation details"
      );
    if (shipper.shipperProfile.assignedHub.toString() !== deliveryInformationId)
      throw new AuthFailureError(
        "You are not assigned to this DeliveryInformation"
      );

    const deliveryInformation = await DeliveryInformation.findById(
      deliveryInformationId
    ).lean();
    if (!deliveryInformation)
      throw new NotFoundError("DeliveryInformation not found");
  ...
  static readDeliveryInformations = async (req) => {
    ...
    const deliveryInformations = await DeliveryInformation.find({
