...
class OrderService {
  ...
  static readOrdersByHub = async (req) => {
    ...
    const user = await User.findById(userId);
    if (!user) throw new AuthFailureError("You are not authenticated!");
    const hub = await Order.findById(hubId);
    if (!hub) throw new NotFoundError("Distribution Hub not found");
    if (
      hub.customerId.toString() !== userId &&
      hub.vendorId.toString() !== userId
    )
      throw new AuthFailureError(
        "You are not authorized to perform this action"
      );

    // 2. Build base filter (by hub)
    const filter = { hubId };

    if (isVendor && !isAdmin && !managesThisHub) {
